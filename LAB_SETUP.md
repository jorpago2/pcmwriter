...
2. Open Kinesis and confirm that all three BPC303 channels appear, then close Kinesis. If a channel is open loop, use **Hardware Dashboard > Stage > Enable closed loop** only after confirming objective clearance and all feedback cables. If its signed position remains outside the configured 0-20 um travel, clear the objective and zero that channel from the BPC303 front panel or Kinesis. PCMWriter changes the feedback mode but never writes zero or a target position.
...
6. With the laser physically disabled, configure the AWG for a 50 ohm load and verify a 0-5 V TTL signal. PCMWriter configures the oscilloscope input to DC/50 ohm during preflight and verifies the readback. Never verify the TTL in Hi-Z because the observed voltage would double.
7. Connect the Stradus Mini-USB port and select `USBHID::201A::1001` in **Diagnostics**. Older heads may also use RS-232 through `ASRL...::INSTR` at 19200 baud, 8-N-1, with no flow control. After each laser power cycle, physically block the beam and run **Laser > Prepare TTL pulse mode**. PCMWriter forces AWG CH1 low/OFF, changes the Stradus to `PUL=1`, verifies `PP`, and returns to `LE=0`. With the TTL cable confirmed LOW, use **Enable TTL emission** to verify `PUL=1`, `EPC=0`, `PP`, and `LE=1`; this second action does not open or change the AWG. Use **Disable TTL emission** to return to verified `LE=0`.
   CW operation does not control the AWG: the **Laser** card in the Hardware Dashboard controls `LE`, selects `PUL=0` after the Stradus startup delay, and sets `LP` directly. If the previous mode was digital modulation, hold its TTL input LOW or block the beam during that transition. Initialize parking power with the beam blocked because the unit may briefly emit at its previous setting.
